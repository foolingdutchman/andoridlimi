...
public class MyExpandableTextView extends TextView {
    ...
    private MyExpandableClickListener mExpandableClickListener;
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, cn.carbs.android.expandabletextview.library.R.styleable.ExpandableTextView);
        if (a == null) {
            return;
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == cn.carbs.android.expandabletextview.library.R.styleable.ExpandableTextView_etv_MaxLinesOnShrink) {
                mMaxLinesOnShrink = a.getInteger(attr, MAX_LINES_ON_SHRINK);
            }else if (attr == cn.carbs.android.expandabletextview.library.R.styleable.ExpandableTextView_etv_EllipsisHint){
                mEllipsisHint = a.getString(attr);
            }else if (attr == cn.carbs.android.expandabletextview.library.R.styleable.ExpandableTextView_etv_ToExpandHint) {
                mToExpandHint = a.getString(attr);
            }else if (attr == cn.carbs.android.expandabletextview.library.R.styleable.ExpandableTextView_etv_ToShrinkHint) {
                mToShrinkHint = a.getString(attr);
            }else if (attr == cn.carbs.android.expandabletextview.library.R.styleable.ExpandableTextView_etv_EnableToggle) {
                mToggleEnable = a.getBoolean(attr, TOGGLE_ENABLE);
            }else if (attr == cn.carbs.android.expandabletextview.library.R.styleable.ExpandableTextView_etv_ToExpandHintShow){
                mShowToExpandHint = a.getBoolean(attr, SHOW_TO_EXPAND_HINT);
            }else if (attr == cn.carbs.android.expandabletextview.library.R.styleable.ExpandableTextView_etv_ToShrinkHintShow){
                mShowToShrinkHint = a.getBoolean(attr, SHOW_TO_SHRINK_HINT);
            }else if (attr == cn.carbs.android.expandabletextview.library.R.styleable.ExpandableTextView_etv_ToExpandHintColor){
                mToExpandHintColor = a.getInteger(attr, TO_EXPAND_HINT_COLOR);
            }else if (attr == cn.carbs.android.expandabletextview.library.R.styleable.ExpandableTextView_etv_ToShrinkHintColor){
                mToShrinkHintColor = a.getInteger(attr, TO_SHRINK_HINT_COLOR);
            }else if (attr == cn.carbs.android.expandabletextview.library.R.styleable.ExpandableTextView_etv_ToExpandHintColorBgPressed){
                mToExpandHintColorBgPressed = a.getInteger(attr, TO_EXPAND_HINT_COLOR_BG_PRESSED);
            }else if (attr == cn.carbs.android.expandabletextview.library.R.styleable.ExpandableTextView_etv_ToShrinkHintColorBgPressed){
                mToShrinkHintColorBgPressed = a.getInteger(attr, TO_SHRINK_HINT_COLOR_BG_PRESSED);
            }else if (attr == cn.carbs.android.expandabletextview.library.R.styleable.ExpandableTextView_etv_InitState){
                mCurrState = a.getInteger(attr, STATE_SHRINK);
            }else if (attr == cn.carbs.android.expandabletextview.library.R.styleable.ExpandableTextView_etv_GapToExpandHint){
                mGapToExpandHint = a.getString(attr);
            }else if (attr == cn.carbs.android.expandabletextview.library.R.styleable.ExpandableTextView_etv_GapToShrinkHint){
    ...
    private void init() {
        ...
        if(TextUtils.isEmpty(mToExpandHint)){
            mToExpandHint = getResources().getString(cn.carbs.android.expandabletextview.library.R.string.to_expand_hint);
        }
        if(TextUtils.isEmpty(mToShrinkHint)){
            mToShrinkHint = getResources().getString(cn.carbs.android.expandabletextview.library.R.string.to_shrink_hint);
        }
        if(mToggleEnable){
            mExpandableClickListener = new MyExpandableClickListener();
    ...
    private CharSequence getNewTextByConfig(){
        ...
        switch (mCurrState){
            case STATE_SHRINK: {
                ...
                int w1=getValidLayout().getWidth();
                Log.i("expandtext","indexStart:----"+indexStart+"; indexEndTrimmed:-----"+indexEndTrimmed);
                Log.i("expandtext","mOrigText:----"+mOrigText);
                int w2=-0;
                if (indexEndTrimmed<indexStart) {
                    w2=(int)(mTextPaint.measureText(mOrigText.subSequence(indexStart, indexEnd).toString()) + 0.5);
                }else
               w2=(int) (mTextPaint.measureText(mOrigText.subSequence(indexStart, indexEndTrimmed).toString()) + 0.5);
                Log.i("expandtext","w1:----"+w1+"; w2:-----"+w2);
                int remainWidth = w1 -w2;
                ...
                if (mShowToExpandHint) {
                    ...
                    ssbShrink.setSpan(new RelativeSizeSpan(0.8f), ssbShrink.length() - getLengthOfString(mToExpandHint), ssbShrink.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            ...
            case STATE_EXPAND: {
                ...
                ssbExpand.setSpan(new RelativeSizeSpan(0.8f), ssbExpand.length() - getLengthOfString(mToShrinkHint), ssbExpand.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                return ssbExpand;
            }
        }
        return mOrigText;
    ...
    public interface OnExpandListener{
        void onExpand(MyExpandableTextView view);
        void onShrink(MyExpandableTextView view);
    }

    private class MyExpandableClickListener implements View.OnClickListener{
        @Override
    ...
    private class TouchableSpan extends ClickableSpan {
        ...
        public void onClick(View widget) {
            if(hasOnClickListeners()
                    && (getOnClickListener(MyExpandableTextView.this) instanceof MyExpandableClickListener)) {
            }else{
                toggle();
            }
        }
