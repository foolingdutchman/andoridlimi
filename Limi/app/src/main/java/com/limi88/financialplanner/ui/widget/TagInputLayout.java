...
public class TagInputLayout extends ViewGroup
        implements TextWatcher, View.OnKeyListener, View.OnClickListener, TextView.OnEditorActionListener {
    ...
    public void afterTextChanged(Editable s) {
        if (s.length() > 0) {
            if (isKeyCharHit(s.charAt(0))) {
                ...
                mInputView.setText("");
                String string = s.subSequence(0, s.length() - 1) + "";
                for (int i = 0; i < getTagArray().length; i++) {
                    if (string.equals(getTagArray()[i] + "")) {
                        return;
                    }
                }
                generateTag(string);
...
//            return;
     ...
     * can be null
     */
    public void generateTag(CharSequence tag) {
...
//            tagLabel = new TextView(getContext());
            tagLabel = (TextView) View.inflate(getContext(), R.layout.floattag_text, null);
...
//        }
    ...
    public class SimpleDecorator implements Decorator {
        ...
        public SimpleDecorator(Context context) {
            ...
            final int m = getPixelSize(context, TypedValue.COMPLEX_UNIT_DIP, 5);
            margin = new int[]{m, m, m, m};
            mRadius = getPixelSize(context, TypedValue.COMPLEX_UNIT_DIP, 15);
            mTagHeight = getPixelSize(context, TypedValue.COMPLEX_UNIT_DIP, 25);
            textColor = new int[]{0xFF009DE9, 0xFFFFFFFF};
        }
        ...
        public Drawable[] getBackgroundDrawable() {
            return new Drawable[]{
                    getContext().getResources().getDrawable(R.drawable.textview_bg_no_sel),
                    getContext().getResources().getDrawable(R.drawable.textview_bg_sel)
//                    newRoundRectShape(0xFF009DE9, mRadius),
//                    newRoundRectShape(0xFF3f51b5, mRadius)
        ...
        public Drawable newRoundRectShape(int color, int radius) {
